...
class VolumeTransformer extends Transform {
	 ...
	 * @param {number} [options.smoothness=0] The volume smoothness
	 * @example
	...
	_transform(chunk, encoding, done) {
		if (this._smoothing > 0 && this.volume !== this._newVolume) {
			if (this.volume < this._newVolume) {
				this.volume = this.volume + this._smoothing >= this._newVolume ? this._newVolume : this.volume + this._smoothing;
			} else if (this.volume > this._newVolume) {
				this.volume = this.volume - this._smoothing <= this._newVolume ? this._newVolume : this.volume - this._smoothing;
			}
		}
